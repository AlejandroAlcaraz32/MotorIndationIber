...
object BronzeCsvValidator extends BronzeValidator with SparkSessionWrapper with Logging {
  override def validate(indationProperties: IndationProperties,
    ...
    val hasCsvHeaders = dataset.fileInput.get.csv.get.header == FirstLine
    ...
    if(hasCsvHeaders) {
      ...
      else if(!isEqualsFieldsName) logger.info("Schema columns names and csv header names are different.")
    }

    val inputDF =
      if(!dataframe.isEmpty && dataset.fileInput.get.csv.get.header == IgnoreHeader) {
