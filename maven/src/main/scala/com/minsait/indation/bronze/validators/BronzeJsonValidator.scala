...
object BronzeJsonValidator extends BronzeValidator with SparkSessionWrapper with Logging{
  override def validate(indationProperties: IndationProperties,
    ...
    if (numWrongRows == 0) {
      ...
      if (numValidRows == 0) {
        isValidFile = false
        logger.warn("All rows in json has different number of columns compared with schema")
      }
    }
