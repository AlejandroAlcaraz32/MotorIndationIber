...
class IngestBatchFileOrcActivitySpec extends AnyFunSuite with MockitoSugar  with BeforeAndAfterAll with BeforeAndAfterEach
  ...
  test("IngestBatchFileActivity#orc ingestion full-snapshot fail-fast ok") {
    val filePath = "pending/source4/20200101_worldCities.snappy.orc"
    ...
    val expectedBronzePath = this.indationProperties.datalake.basePath +
      "bronze/source4/full-snapshot-worldcities-orc/2020/01/01/20200101_worldCities.snappy.orc"

    val expectedSilverPath = this.indationProperties.datalake.basePath + "silver/public/database4/full_snapshot_worldcities_orc"

    val bronzeDF = spark.read.format("orc").load(expectedBronzePath)
    val silverDF = spark.sql("select * from database4.full_snapshot_worldcities_orc")
    ...
    assertActivityStatistics(activityStatistics, filePath, ActivityResults.Ingested_Without_Errors,None,
      ...
      Some(ActivitySilverPersistence(Some("database4"),Some("full_snapshot_worldcities_orc"),None,Some(0L),
        Some("full_snapshot_worldcities_orc_historical"),None,Some(0L))))

    assertResult(expectedSilverPath + this.historicalSuffix)(activityStatistics.output_paths.silver_historical.getOrElse(None))

    assertActivityDataset(activityStatistics, "full-snapshot-worldcities-orc", IngestionTypes.FullSnapshot, ValidationTypes.FailFast)
  ...
  test("IngestBatchFileActivity#orc ingestion full-snapshot fail-fast schema-mismatch") {
    val filePath = "pending/source4/20200101_worldCities.zlib.orc"
    val ingestBatchActivity = new IngestBatchFileActivity(indationProperties)
    ingestBatchActivity.executeWithFile(filePath, adfRunId)

    val expectedSchemaMismatch = this.indationProperties.landing.basePath  +
      "schema-mismatch/source4/full-snapshot-worldcities-subset-orc/2020/01/01/20200101_worldCities.zlib.orc"

    val notExpectedBronzePath = this.indationProperties.datalake.basePath +
      "bronze/source4/full-snapshot-worldcities-subset-orc/2020/01/01/20200101_worldCities.zlib.orc"

    val notExpectedSilverPath = this.indationProperties.datalake.basePath + "silver/public/database4/full_snapshot_worldcities_subset_orc"
    ...
    assertActivityDataset(activityStatistics, "full-snapshot-worldcities-subset-orc", IngestionTypes.FullSnapshot, ValidationTypes.FailFast)
