...
class IngestBatchFileParquetActivitySpec extends AnyFunSuite with MockitoSugar  with BeforeAndAfterAll with BeforeAndAfterEach
  ...
  test("IngestBatchFileActivity#parquet ingestion full-snapshot fail-fast ok") {
    val filePath = "pending/source3/20200101_worldCities.snappy.parquet"
    ...
    val expectedBronzePath = this.indationProperties.datalake.basePath +
      "bronze/source3/full-snapshot-worldcities-parquet/2020/01/01/20200101_worldCities.snappy.parquet"

    val expectedSilverPath = this.indationProperties.datalake.basePath + "silver/public/database3/full_snapshot_worldcities_parquet"

    val bronzeDF = spark.read.format("parquet").load(expectedBronzePath)
    val silverDF = spark.sql("select * from database3.full_snapshot_worldcities_parquet")
    ...
    assertActivityStatistics(activityStatistics, filePath, ActivityResults.Ingested_Without_Errors,None,
      ...
      Some(ActivitySilverPersistence(Some("database3"),Some("full_snapshot_worldcities_parquet"),None,Some(0L),
        Some("full_snapshot_worldcities_parquet_historical"),None,Some(0L))))

    assertResult(expectedSilverPath + this.historicalSuffix)(activityStatistics.output_paths.silver_historical.getOrElse(None))

    assertActivityDataset(activityStatistics, "full-snapshot-worldcities-parquet", IngestionTypes.FullSnapshot, ValidationTypes.FailFast)
  ...
  test("IngestBatchFileActivity#parquet ingestion full-snapshot fail-fast schema-mismatch") {
    val filePath = "pending/source3/20200101_worldCities_name.gz.parquet"
    val ingestBatchActivity = new IngestBatchFileActivity(indationProperties)
    ingestBatchActivity.executeWithFile(filePath, adfRunId)

    val expectedSchemaMismatch = this.indationProperties.landing.basePath  +
      "schema-mismatch/source3/full-snapshot-worldcities-subset-parquet/2020/01/01/20200101_worldCities_name.gz.parquet"

    val notExpectedBronzePath = this.indationProperties.datalake.basePath +
      "bronze/source3/full-snapshot-worldcities-subset-parquet/2020/01/01/20200101_worldCities_name.gz.parquet"

    val notExpectedSilverPath = this.indationProperties.datalake.basePath + "silver/public/database3/full_snapshot_worldcities_subset_parquet"
    ...
    assertActivityDataset(activityStatistics, "full-snapshot-worldcities-subset-parquet", IngestionTypes.FullSnapshot, ValidationTypes.FailFast)
